...
export class EventsService {
   ...
   *
   * @async
   * @param {EventAPISource} source - The source from which 
   * to fetch the data.
   * @param {number} id - The ID of the item to retrieve.
   * @returns {Promise<{ 
   * data: LeaderDataDto | TimepadDataDto | null 
   * }>} The data object fetched from the source, or null 
   * if not found.
   */
  ...
  /**
   * Retrieves a paginated list of events from Leader and 
   * Timepad sources.
   *
   * @async
   * @param {GetEventListQueryDto} query - The query parameters 
   * including pagination and filters.
   * @returns {Promise<object>} An object containing event data 
  ...
  /**
   * Retrieves a list of events along with metadata from the 
   * specified source.
   *
   * @async
   * @param {EventAPISource} source - The source from which to 
   * fetch the events.
   * @param {GetEventListQueryDto} query - The query parameters 
   * for filtering and pagination.
   * @returns {Promise<EventsListResultDto>} The events data 
   * along with pagination metadata.
   * @throws {NotFoundException} Throws if no events are found 
  ...
  /**
   * Retrieves the total number of events from both Leader and 
   * Timepad sources based on the query.
   *
   * @async
   * @param {GetEventListQueryDto} query - The query parameters 
   ...
   * }>} An object containing event counts from both sources.
   * @private
   */
  ...
  /**
   * Calculates the total number of pages based on total items 
  ...
  /**
   * Fetches events from Leader and Timepad services based on 
   * batch data and query parameters.
   *
   * @async
   * @param {{
   *   firstAmount: number;
   ...
   *   secondAmount: number;
   *   secondSkip: number;
   * }} batchData - Object containing amounts and offsets for 
   * both sources.
   * @param {GetEventListQueryDto} query - The query parameters 
   * for filtering and pagination.
   * @returns {Promise<UnifiedEventDto[]>} A flattened array of 
   * events fetched from both sources.
   * @private
   */
  ...
  /**
   * Calculates how many items to take and skip from two APIs 
   * for pagination, balancing the distribution.
   *
   * @param {number} limit - The number of items per page.
   * @param {number} page - The current page number.
   * @param {number} api1Total - Total available items from the 
   * first API.
   * @param {number} api2Total - Total available items from the 
   * second API.
   * @returns {{
   *   firstSkip: number;
   *   firstAmount: number;
   ...
   *   secondAmount: number;
   *   isEmpty: boolean;
   * }} The calculated skip and take amounts for both APIs and 
   * a flag indicating if the batch is empty.
   * @private
   */
  ...
  /**
   * Distributes the number of items to take from two sources 
   * given their remaining counts and a limit.
   * Attempts to evenly split the limit, then fills the remainder 
   * from whichever source has available items.
   *
   * @param {number} limit - Total number of items to take.
   * @param {number} half - Half of the limit, used as initial 
   * allocation for the first source.
   * @param {number} rest1 - Remaining items available in the 
   * first source.
   * @param {number} rest2 - Remaining items available in the 
   * second source.
   * @returns {{ take1: number; take2: number }} The number of 
   * items to take from each source.
   * @private
   */
