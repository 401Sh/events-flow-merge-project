...
export class OAuthService {
  ...
  /**
   * Constructs the OAuth authorization redirect URL for a given 
   * event API source.
   *
   * @param {EventAPISource} source - The source for which to generate 
   * the redirect URL.
   ...
   *
   * @async
   * @param {CallbackDto} query - The callback query containing 
   * the authorization code.
   * @returns {Promise<CallbackResultDto>} The result data containing 
   * the leader access token.
   */
   ...
   *
   * @async
   * @param {string} refreshToken - The refresh token used to 
   * obtain a new access token.
   * @throws {UnauthorizedException} Throws if the refresh token 
   * is missing.
   * @returns {Promise<CallbackResultDto>} The new token data 
   * returned from the leader service.
   */
  ...
  /**
   * Returns the OAuth redirect URL components based on the 
   * given event API source.
   *
   * @param {EventAPISource} source - The event API source to 
   * determine URL parts for.
   * @returns {{ baseURL: string; clientId: string; type: string }} 
   * An object containing the base URL, client ID, and response 
   * type for the OAuth flow.
   * @private
   */
