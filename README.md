...
- [X] Запись и отмена записи на мероприятие Leader-ID
- [ ] Запись и отмена записи на мероприятие Timepad
...
В данный момент проект не может работать без предоставления токена доступа к API timepad и client_id с secret_id от API leader-id, а также без Postgis базы данных (проекту также требуется S3 хранилище и почтовый провайдер для работы auth модуля и загрузки изображений соответственно).
